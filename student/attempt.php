...
if (is_post() && (string)request('auto_submit') === '1') {
    ...
    db()->prepare('UPDATE exam_attempts SET status="auto_submitted", submitted_at=NOW() WHERE exam_id=? AND student_id=?')
        ->execute([$examId, $sid]);
    flash('info', 'Time expired — paper submitted automatically.');
    redirect('/student/exams.php');
}
...
if (is_post() && (string)request('action') === 'final_submit') {
    ...
    flash('success', $auto ? 'Time expired — paper submitted automatically.' : 'Paper submitted successfully.');
    redirect('/student/exams.php');
}
...
// Save answers while navigating (next/prev within section keeps form post optional — use GET nav)
if (is_post() && (string)request('action') === 'save_answers') {
    ...
    redirect('/student/attempt.php?exam_id=' . $examId . '&section_id=' . (int)request('section_id') . '&q=' . $qIndex);
}
...
// Completion interstitial after section
if ($showCompletion && $sectionParam) {
    $summary = mcq_section_summary($examId, $sid, $sectionParam);
    $pageTitle = 'Section complete';
    $activeNav = 'exams';
    $bodyClass = 'exam-taking';
    require dirname(__DIR__) . '/includes/header.php';
    $remaining = false;
    foreach ($sections as $sec) {
        if (($progress[(int)$sec['id']] ?? 'pending') !== 'completed' && (int)$sec['id'] !== $sectionParam) {
            $remaining = true;
            break;
        }
    }
    // refresh progress after mark
    $progress[(int)$sectionParam] = 'completed';
    $allDone = all_sections_completed($examId, $sid);
    ?>
    <div class="exam-gate-card panel">
        <div class="exam-gate-header" style="background:linear-gradient(135deg,#0f766e,#134e4a)">
            <h2>Successfully Completed Section</h2>
        </div>
        <div class="exam-gate-body">
            <h3 style="margin-top:0">MCQ Sections Summary</h3>
            <div class="summary-banner">
                <div><span>Total MCQ Marks</span><strong><?= e((string)$summary['mcq_obtained']) ?> / <?= e((string)$summary['mcq_total']) ?></strong></div>
                <div><span>Correct Answers</span><strong><?= (int)$summary['correct'] ?></strong></div>
                <div><span>Total MCQ Questions</span><strong><?= (int)$summary['mcq_q'] ?></strong></div>
            </div>
            <?php if ($allDone): ?>
...
if ($allDone && !$currentSection) {
    ...
    ?>
    <div class="exam-gate-card panel">
        <div class="exam-gate-header"><h2>Successfully Completed Paper</h2></div>
        <div class="exam-gate-body">
            <p>All sections are complete. Submit your paper to finish.</p>
            <form method="post">
                <?= csrf_field() ?>
                <input type="hidden" name="action" value="final_submit">
                <div id="examAttempt" data-exam-id="<?= (int)$examId ?>" data-ends-at="<?= strtotime($attempt['ends_at']) ?>" hidden></div>
                <button class="btn btn-primary" type="submit" style="width:100%">FINAL SUBMIT PAPER</button>
            </form>
        </div>
    </div>
    <?php
    ...
    <form method="post" id="examForm">
        ...
        <article class="question-card" id="q-<?= $qid ?>">
            ...
            <div class="question-nav">
                ...
                <?php else: ?>
                    <button class="btn btn-primary" type="submit" onclick="document.getElementById('formAction').value='complete_section'; return confirm('Complete this section?');">Complete section</button>
                <?php endif; ?>
            </div>
